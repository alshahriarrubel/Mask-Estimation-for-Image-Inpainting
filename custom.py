...
class frequencyFeature(nn.Module):
	...
	def forward(self, im):
		...
		#option 1
		f_threshold = 10	
		filtermask = torch.ones(im.shape[2],im.shape[3]).to(config.DEVICE)
...
class VisionTransformer(nn.Module):
	...
	def __init__(
			self,
			img_size=config.INPUT_IMAGE_WIDTH,
			patch_size=16,
			in_chans=1,			  
			embed_dim=256,
			depth=16,
	...
	def forward(self, x):
		...
		for idx in range(int(math.log2(self.img_size//x.shape[3]))):
			in_size = x.shape[1]
			out_size = in_size//4
			up = UpBlock(in_size,out_size).to(config.DEVICE)
			x = up(x)
		x = torch.sigmoid(x)
		x = 1 - x
		#x = (x>0.75) * 1.0
		print('After upsampling: ',x.shape)
		print('LAST OUTPUT OF MODEL')
		print(x[0,0,:4,:4])
		"""
		in_size = x.shape[1]
		out_size = in_size//4
